...
@scorer(metrics=[])
def groundedness_scorer_cot(judge_service: LLMProviderService = None) -> Scorer:
    """
    ...
    judge_service = judge_service if judge_service else AnthropicService()

    async def score(state: TaskState, target: Target) -> Score:
        raw_source_data = state.metadata.get("raw_input_data")
        generated_copy_text = state.metadata["generated_copy"]

        if not raw_source_data or not generated_copy_text:
            return Score(
                value=0.0,
                explanation="Missing source data or generated output in state context.",
            )
        ...
        if not checks:
            return Score(
                value=1.0,
                explanation="Passed audit: The text contained no testable factual assertions.",
            )
...
def structure_scorer() -> Scorer:
    ...
    async def score(state: TaskState, target: Target) -> Score:
        ...
        try:
            ...
            return Score(value=0.0, explanation=f"Schema validation failed: {str(e)}")
