...
class MarketingCopy(BaseModel):
    ...
    hero_paragraph: str = Field(
        description="A 3-sentence description emphasizing the location and vibe."
    )
    amenity_highlights: List[str] = Field(
        description="Exactly 3 key highlights mapped to verified amenities."
    )
    guest_expectations: str = Field(
        description="A brief summary of check-in/out and key rules."
    )
